...
function UsageList() {
  ...
  const handleCancel = (reservationId) => {
    axios
      .post(
        "https://withpet.site/api/v1/reservation/user/cancel-reservation",
        { reservationId },
  ...
  const handleWaitRefund = (reservationId) => {
    axios
      .post(
        "https://withpet.info/payment/refund",
        { reservationId },
        { withCredentials: true }
      )
      .then(() => {
        ...
        alert("예약이 취소되었습니다.");
        setPayedList(
          payedList.filter((item) => item.reservationId !== reservationId)
        );
      });
  };
  ...
  const handleApproveRefund = (reservationId) => {
    axios
      ...
      .then(() => {
        ...
        alert("예약이 취소되었습니다.");
        setApproveList(
          approveList.filter((item) => item.reservationId !== reservationId)
        );
      });
  };
  ...
  const handleDone = (reservationId) => {
    axios
      .post(
        "https://withpet.site/api/v1/reservation/user/done-reservation",
        { reservationId },
        { withCredentials: true }
      )
      .then(() => {
  ...
  useEffect(() => {
    ...
    axios
      ...
      .then(() => {
        ...
        // 결제 대기에서 예약 대기로 이동
        setWaitList(
          waitList.filter((item) => item.reservationId !== saveReservationId)
        );
        const select = waitList.filter(
          (item) => item.reservationId === saveReservationId
        );
  ...
  return (
    ...
    >
      ...
      <WaitList
        list={payedList}
        handleCancel={handleWaitRefund}
        stepValue="2"
      />
      <WaitList
        list={approveList}
        handleCancel={handleApproveRefund}
        stepValue="3"
      />
      <WaitList
        list={useList}
