...
import { useGetPetsitterReservation } from "../../hooks/usePetsitterReservationMutate";
function PetsitterCalendar() {
  ...
  const { data: petsitterReservationData } =
    useGetPetsitterReservation(selectedMonth);
  console.log("petsitterReservationData:", petsitterReservationData);
  console.log("selectMonth", selectedMonth);

  useEffect(() => {
    if (petsitterReservationData) {
      const filtered = petsitterReservationData.filter(
        (item) => item.reservationStatus !== "CANCEL"
      );
      const temp = filtered.map((item) => ({
        start: new Date(item.reservationCheckIn),
        end: new Date(item.reservationCheckOut),
  ...
  if (printBody[0] === "main") {
    ...
    print = (
      <PetsitterEvaluation id={printBody[1]} setPrintBody={setPrintBody} />
    );
  } else {
